<?php

namespace Wrapper;

class CallType
{
    public const INDIVIDUAL_CONSUMER_TRACE_LITE         = "ConsumerTraceLite";
    public const INDIVIDUAL_CONSUMER_TRACE              = "ConsumerTrace";
    public const INDIVIDUAL_CONSUMER_TRACE_PRO          = "ConsumerTracePro";
    public const INDIVIDUAL_TELEPHONE_TRACE             = "TelephoneTrace";
    public const INDIVIDUAL_CONSUMER_TRACE_DOV          = "ConsumerTraceDOV";
    public const INDIVIDUAL_CONSUMER_TRACE_DOV_RESULT   = "ConsumerTraceDOVGetResult";
}